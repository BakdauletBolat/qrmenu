...
<template>
  <article class="bg-white relative p-4 gap-4 rounded-xl flex">
        <RouterLink :to="{
          name: 'foods',
          params: {
            categoryId: item.id
             ...
             <ImageComponent class="h-full w-full absolute rounded-xl" :url="item.images.length > 0 ? item.images[0].image : undefined" :alt="item.name"></ImageComponent>
        </RouterLink>
        <div class="flex flex-col z-10 justify-between w-full">
            <div>
                <h3>{{ item.name }}</h3>
